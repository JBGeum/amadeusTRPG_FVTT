...
- **목표**: "비밀 플롯 → 동시 공개 → 순서 배정" 메커니즘을 독립 도구로 구현.
- **범위 밖**: Foundry 기본 전투 트래커(Combat/Combatant) 통합, 토큰/씬 인카운터 의존 — 사용하지 않는다. SCSS 디자인 리뉴얼과 별개(스타일은 셀렉터만 제공).
- **참가자**: 기본은 PC(각 플레이어가 자신의 character로 플롯). GM은 패널에서 NPC를 추가하고 그 주사위를 직접 입력할 수 있다.

## 핵심 규칙 (합의됨)

- 플롯값은 **1~6 정수 중 하나를 선택**(주사위를 굴리지 않음).
...
| 파일 | 책임 | 의존 | 테스트 |
|------|------|------|--------|
| `order.mjs` | **순수**: `computeOrder(entries)` — 플롯값 오름차순 정렬 + 동률 그룹핑 | 없음 | Vitest |
...
| `plot-prompt.mjs` | AppV2 Application: 플레이어 플롯 UI(1~6 선택·제출) | socket | 수동 |
...
- `templates/initiative/plot-prompt.html` — 플레이어 플롯 프롬프트
...
- `plot-start` (GM→전체): `{ sessionId }`. 참가 character를 소유한 플레이어 클라이언트가 플롯 프롬프트를 자동으로 연다.
- `plot-submit` (플레이어→GM): `{ sessionId, actorId, userId, value }`. **GM만 처리** — 세션에 값 기록, 패널 갱신. (다른 클라이언트는 무시 → UI 비밀.)
- `plot-reveal` (GM→전체): `{ sessionId }`. 플레이어 프롬프트 닫힘. 영구 기록은 결과 챗 카드.
- `plot-cancel` (GM→전체): `{ sessionId }`. 세션 취소/프롬프트 닫힘.
...
2. GM **"플롯 시작"** → 세션 active → `plot-start` 브로드캐스트 → 각 플레이어에게 플롯 프롬프트(1~6 선택 + 제출).
3. 플레이어 제출 →
   - `plot-submit`을 GM에 전송 → GM 패널에 해당 참가자의 **실제 값** 표시(GM만).
...
5. GM **"공개"** → `computeOrder` 실행 → 공개 **결과 챗 카드**(`initiative-result.html`: 순서·값·동률 그룹) 게시 + `plot-reveal` 브로드캐스트(플레이어 프롬프트 닫힘) → 세션 `revealed`.
6. GM **"초기화"**로 새 세션 시작.
...
- **플레이어 재접속**: 프롬프트가 닫힌다 → GM **"재요청"** 버튼으로 `plot-start`를 재송신해 프롬프트를 다시 연다.
...
- **소켓/UI 수동 검증**: GM + 플레이어 멀티 클라이언트로 플롯 시작/제출/플롯완료 카드/공개/결과 카드/재요청/초기화 시나리오.
